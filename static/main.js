...
// --- 新增：绑定每页显示数量选择器事件 ---
    ...
    itemsPerPage = parseInt(this.value, 10); // 将选中的值转为数字
    currentPage = 1; // 切换每页数量时，重置到第一页
    renderFileList(); // 重新渲染当前页
});
// --- 新增：检查是否为管理员，并添加管理员按钮 ---
// 获取当前登录用户的权限 (这个变量需要后端在 index.html 中传递)
if (window.GLOBAL_IS_ADMIN) {
    // 创建管理员按钮
    const adminButton = document.createElement('button');
    adminButton.textContent = '管理员面板';
    adminButton.className = 'action-btn';
    adminButton.style.position = 'absolute';
    adminButton.style.top = '70px';
    adminButton.style.right = '20px';
    adminButton.addEventListener('click', function() {
        window.location.href = '/admin/users';
    });
    document.querySelector('.container').appendChild(adminButton);
   }
    // 初始加载文件列表
    loadFileList();
});