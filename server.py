...
class User(UserMixin):
    def __init__(self, username):
        self.id = username # UserMixin 要求 id 属性
# --- 用户加载回调 ---
...
def index():
    ...
    return render_template('index.html', is_authenticated=current_user.is_authenticated, csrf_token=generate_csrf())
# --- 路由：登录 ---
...
def login():
    ...
    if request.method == 'POST':
        ...
        if username == USER_USERNAME and USER_PASSWORD_HASH:
            # 使用 bcrypt 验证密码
        ...
        # 不需要在这里 return，继续执行到下面的 return
    # --- 处理 GET 请求 或 POST 失败 ---
    # 无论是首次访问 (GET) 还是登录失败 (POST)，都渲染登录页并传递 csrf_token
...
def upload_file_async():
    ...
    try:
        ...
        file.save(temp_filepath)
        # 提交异步任务
        task = async_upload_file.delay(temp_filepath, final_filepath, filename) # 传递 filename
...
def get_task_status(task_id):
    ...
    task = async_upload_file.AsyncResult(task_id)
